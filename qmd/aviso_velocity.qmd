...
- need to have downloaded `ffmpeg` to animate vectors
  ...
  - other option is to save as gif using `camcorder`, then convert to mp4 using `ffmpeg` from shell
    - ffmpeg -i example.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" test.mp4
...
# aviso_path_full <- dir_ls(here("data", "raw", "copernicus", "loop_current")) 
...
aviso_path <-
  aviso_path_full %>%
  Filter(\(.x) str_detect(.x, year_to_load), .) %T>%
  # .[6:9] %T>% # add months to run
  {
    .[] %>%
...
dist_avg_month <-
  ...
  summarise(
    ...
    vel_mag_sd = sd(vel_mag, na.rm = TRUE),
    vel_mag = mean(vel_mag, na.rm = TRUE),
  ) %>%
  mutate(
    vel_min = vel_mag - vel_mag_sd,
    vel_max = vel_mag + vel_mag_sd,
  )
...
dist_from_shore2 %>%
  ...
  # filter(str_detect(line_name, "LK")) %>%
  ggplot(aes(x = dist, y = vel_mag, color = date)) +
  geom_ribbon(data = dist_avg_month, aes(ymin = vel_min, ymax = vel_max), 
              color = "grey50", fill = "grey50", alpha = 0.5) +
  ...
  # facet_wrap(months ~ line_name, ncol = 4)
```
...
if (FALSE) {
  ...
  trasect_plt <- 
    ...
    ggplot(aes(x = dist, y = vel_mag, color = as.factor(line_name), group = date2)) +
    geom_hline(yintercept = 0.5, color = "red") +
    geom_path() +
    geom_point(size = 1) +
    labs(
    ...
    scale_y_continuous(
      guide    = guide_axis(
        title = expression(Velocity ~ (m ~ s^{-1}))
        ) 
    ) +
    coord_cartesian(expand = FALSE) +
    theme_bw() +
  ...
  # ---- save plot ---- #
  save_gg(
    plt            = trasect_plt, # edit if want specific plot
    save_location  = animate_save,
    save_name      = "test_transect",
    verbose        = TRUE,
    # overwrite      = FALSE,
    overwrite      = TRUE,
...
slice_by      <- 5
length_divide <- 2 # not used in current plotting, used in WIP
filt <- 
  . %>% 
  filter()
  # filter(date == first(date))
  # filter(between(as_date(date), as_date(first(date)), as_date(first(date)) + 4))
...
# plt_mag_vel_vect <-
  ...
  scale_fill_gradientn(
    name     = "Current",
    colours  = oce::oceColorsVelocity(120)[-c(48:73)], # edit colors
    limits   = c(0, 2),
  ...
  scale_color_gradientn(
    name     = "Current",
    colours  = oce::oce.colorsPalette(120)[-c(48:72)], # edit colors
    limits   = c(0, 2),
...
# save animation
anim_save(
  file      = glue("mag_vel_vect2_{date_suffix}_loop2.mp4"), 
...
```{r}
here("data", "plots", "aviso_test2") %>%  
camcorder::gg_record(dpi = 600, width = 12)
...
use `st_intersection(fl_keys_poly)` to crop to Florida Keys Polygon


# ---- Create Florida Keys Polygon ----

Load or create a polygon for the Florida Keys only. This will remove everything 
north and east of the keys, leaving just the waters closest to the keys  

Stations are based on a classification from Enrique and Luke. Here, we are 
selecting only the stations within the Florida Keys

```{r edit-transects}
fl_poly_file <- here(shp_save, "fl_keys_polygon.shp")
# fl_poly_file <- rstudioapi::selectFile()

if (exists("cloud_dir")) {
  fl_poly_file <- here(cloud_dir, "data", "raw", "fl_keys_polygon.shp")
}

# stations of the Florida Keys
# stations_path <- rstudioapi::selectFile()
stations_path <- 
  here("data", "raw") %>%
  dir_ls(regexp = "fknms")
...
# load or create Florida Keys polygon
if (file_exists(fl_poly_file)) {
  message("Reading transects shapefile")
  fl_keys_poly <- st_read(fl_poly_file)
} else {
  ...
  message(paste0("Saving Florida Polygon as `", basename(fl_poly_file), "`."))
...
```{r}
# crop the sf obj of vectors to the polygon
keys_vel <-
  aviso_grid_tibble %>%
  filter(date == first(date)) %>%
  st_as_sf(coords = c("lon", "lat"), remove = FALSE) %>%
  st_set_crs(st_crs(4326)) %>%
  st_intersection(fl_keys_poly) %>%
  as_tibble()

# another method using raster masking, this leaves masked values as NA
test <-
  multi_rast %>%
  terra::subset(1:4) %>%
  mask(fl_keys_poly) %>%
...
ggplot() +
  ...
  # velocity pixels
  # geom_sf(data = keys_vel)
  geom_tile(data = keys_vel, aes(x = lon, y = lat, fill = vel_mag),
            na.rm = TRUE) +
  ...
  # velocity vectors
  # using sf and intersection method
  metR::geom_arrow(
    data = keys_vel,
    skip = 0,
    aes(
...
Load or create a transect along the keys at the outer most stations to see
when these stations are in/out of the Florida Current
...
```{r extract-along-transect}
# from raster, each variable for each day is a layer, so need to divide by 
# number of variables to get number of dates
n_layers <- nlyr(multi_rast) / length(dat_grid)
...
vel_ext_transect <-
  # extract raster cells along the line
  terra::extract(multi_rast, linestring, xy = TRUE) %>%
...
vel_ext_transect %>%
  ...
  # geom_line(aes(x = lon, y = lat)) +
   # transects
  geom_tile(aes(x = lon, y = lat, fill = vel_mag),
            na.rm = TRUE) +
  geom_sf(data = linestring, color = "red")  +
