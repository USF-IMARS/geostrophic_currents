...
Need to select the correct url for the time period being selected.

`GOMb0.01/reanalysis/data/` - 2001-01-01 - 2024-09-01 18:00

`GOMe0.01/expt_02.8/data/archive/` - 2024-09-02 19:00 - present

```{r constants}
# ---- catalog url ---- #
# 2001-01-01 - 2024-09-01
hycom_url <- "https://tds.hycom.org/thredds/catalog/datasets/GOMb0.01/reanalysis/data/"

if (min_year >= 2024) {
  # 2024-09-02 -
  hycom_url <- "https://tds.hycom.org/thredds/catalog/datasets/GOMe0.01/expt_02.8/data/archive/"
}
...
if (exists("cloud_dir")) {
  hycom_info <-
    here(
      cloud_dir,
      "data",
      # "hycom_100deg_download_instructions_3.csv"
      glue("hycom_100deg_download_instructions_{min_year}_{max_year}.csv")
      )
} else {
  hycom_info <- 
    here(
...
if (!file_exists(hycom_info)) {
  ...
  message("1. Searching for every file name")
  ...
  search_individual_files <-
    ...
    filter( 
      year >= min_year
      & if (is.null(max_year)) TRUE else year <= max_year
      ) %>% 
  ...
  message("2. Searching for file location")
  ...
  message("3. Extracting date from URL")
  ...
  message("4. Final setup of date, file path and file name")
  final_download_info <-
    extract_date_location %>%
    mutate(
      .keep = "used",
  ...
  message(glue("5. Write instructions to {hycom_info}"))
...
# dir_create(dwnld_path) # maybe put this after path_save because need year and month
...
path_save <- 
  final_download_info %>%
  mutate(
    year = ymd(str_extract(date, "\\d{4}-\\d{2}-\\d{2}")),
    month = format(year, "%m"),
    year = year(year),
    yrmth = ymd(str_extract(date, "\\d{4}-\\d{2}-\\d{2}")),
    yrmth = format(yrmth, "%Y/%m/"),
    file_name = paste0(yrmth, file_name)
  ) %>% 
  mutate(
    final_path = map2_chr(
      final_path,
...
# create folder for each year month pair
path_save %>% 
 distinct(yrmth, year, month) %$%
  walk(
    yrmth,
    \(.x) {
      here(dwnld_path, .x) %>% 
...
### 3.3.2 Download HYCOM Data One at a Time

```{r single-download}
path_save %>% 
  mutate(
    date2 = str_remove(date, "T.*") %>% ymd(),
    rows = row_number(),
    max_r = nrow(path_save)) %>% 
  # filter dates
  # filter(date2 %in% min_max_date) %$%
  # filter(year(date2) == 2010) %>% 
  # filter(between(month(date2), 5, 8)) %>%
  # slice(1:5) %>%
  filter() %T>%
  {
    
    message(paste("Downloading", nrow(.), "files"))
    message(paste("Date ranges:", paste(range(.$date2), collapse = " - ")))
  } %$%
  pwalk(
    list(
      final_path,
    ...
    \(.url, .file_name, idx, max_row) {
      message(paste(idx, "of", max_row))
      message(paste("HYCOM URL:", .url))
      message(paste("\nDate:", ymd(str_extract(basename(.file_name), "\\d+_\\d+_\\d+")), "\n"))
      message(paste("Save Path:", dirname(here(dwnld_path, .file_name))))
      message(paste("File name:", basename(.file_name), "\n"))
...
# check file size is less than than 1 Kb, then delete 
here(dwnld_path) %>%
  dir_ls(regexp = "\\.nc4", recurse = TRUE) %>%
  file_info() %>%
  dplyr::arrange(size) %>%
  dplyr::filter(size < fs_bytes("1Kib")) %>%
  pull(path)  %T>% 
  print() %>%
  {
    if (length(.) > 0 && menu(c("Keep small files", "Delete")) == 2) {
      file_delete(.)
    }
  }
...
here(dwnld_path) %>%
  ...
  mutate(
    ...
    yrmth = str_extract(file, "\\d{4}_\\d{2}"),
    yrmth = str_replace(yrmth, "_", "/")
  ) %>%
  count(year) %T>% 
  print() 
```
...
if (FALSE) {
  ...
  with_progress({
    ...
    filt_download %$%
      future_walk2(final_path, file_name, \(.url, .file_name) {
        p()
        message(paste("HYCOM URL:", .url))
        message(paste("\nSave Path:", dirname(here(
          dwnld_path, .file_name
        ))))
        message(paste("File name:", basename(.file_name), "\n"))
