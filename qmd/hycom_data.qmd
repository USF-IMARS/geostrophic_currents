...
for (i in cruise_id) {
  ...
  if (dir_exists(folder_name)) {
    print(glue("Cruise {i} already exists. Skipping!"))  
    next
  }
  ...
  # folder_name <- glue("camcorder_{i}")
  
  # shelf(camcorder)
  # camcorder::gg_record(here("data", "plots", folder_name), dpi = 600)
  # camcorder::gg_stop_recording()
  camcorder::gg_playback(
    frame_duration       = 0.7, 
    first_image_duration = 10,
  ...
  folder_name %>% 
    ...
    mutate(
      ...
      # new_file = dirname(old_file),
      # new_file = glue("{new_file}")
    ) %$%
    map2(
      old_file,
      file_name,
